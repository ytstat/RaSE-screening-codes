# This is the R script used to compare the performance of various screening methods in Example 4 in [TF21]
# 'v.list' records the order of p features provided by different methods.
# 'v.list[[1]]' represents results of Example 4 in [TF21]
# [TF21] sets seed = 1:200, then calculate the median MMS to obtain the results in Table 3.
#
# Reference:
...
# -------------------------------------------------------
# Example 4: continuous response, the interaction model
# -------------------------------------------------------
n <- 300
p <- 1000
train.data <- RaModel(model.type = "screening", model.no = 4, n = 300, p = 1000)
xtrain <- train.data$x
ytrain <- train.data$y
xtrain <- scale(xtrain)
D <- floor(sqrt(n))
