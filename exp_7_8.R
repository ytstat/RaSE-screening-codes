# This is the R script used to compare the performance of various screening methods in Examples 7 and 8 in [TF21]
# 'v.list' records the order of p features provided by different methods.
# 'v.list[[1]]' and 'v.list[[2]]' represent results of Examples 7 and 8 in [TF21], respectively
# [TF21] sets seed = 1:200, then calculate the median MMS to obtain the results in Table 6.
#
# Reference:
...
n <- length(rat$y)
p <- 2000
train.data <- RaModel(model.no = 7, model.type = "screening", n = 200, p = 2000)
xtrain <- train.data$x
ytrain <- train.data$y
xtrain <- scale(xtrain)
D <- floor(sqrt(n))
...
n <- 200
p <- 2000
train.data <- RaModel(model.type = "screening", model.no = 8, n = 200, p = 2000)
xtrain <- train.data$x
ytrain <- train.data$y
xtrain <- scale(xtrain)
D <- floor(sqrt(n))
